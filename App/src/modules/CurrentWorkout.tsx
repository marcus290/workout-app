...
export const CurrentWorkout: React.FC<Props> = observer(
    ...
    const dateKey = `${year}-${month}-${day}`;
        ...
        return (
            ...
            <ScrollView 
           keyboardShouldPersistTaps="always" contentContainerStyle={styles.scrollContainer}>
                {(isCurrentWorkout 
                ...
                <Button title="SAVE" onPress={() => { 
                    if (isCurrentWorkout) {
                        rootStore.workoutStore.history[dayjs(new Date(+(new Date()) - Math.floor(Math.random()*10000000000))).format('YYYY-MM-DD')] = rootStore.workoutStore.currentExercise;
                        rootStore.workoutStore.currentExercise = [];
                    }
                    history.push("./"); 
